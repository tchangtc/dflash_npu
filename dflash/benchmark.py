...
def _dist_init(torch_dist) -> None:
    if "RANK" not in os.environ:
        ...
        return
    from .device import get_dist_backend
    torch_dist.init_process_group(backend=get_dist_backend(), init_method="env://")
...
def _get_transformers_attn_impl() -> str:
    ...
    # NPU does not support the NVIDIA flash_attn package; use SDPA instead
    if get_device_type() == "npu":
        logger.info("NPU detected: using sdpa attention implementation")
        return "sdpa"
...
def _run_transformers(args: argparse.Namespace) -> None:
    ...
    set_device(_dist_local_rank())
    device = get_device(_dist_local_rank())
